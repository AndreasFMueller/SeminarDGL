...
Wir k"onnen die Differentialgleichung (\ref{numerik:eulerdgl}) nochmals
nach $x$ ableiten:
\begin{align}
y'(x)
&=
...
\end{equation}
wobei wir f"ur $y''(x)$ die Gleichung (\ref{numerik:2abl}) verwendet haben.
...
gegen"uber dem Euler-Verfahren ist konsistent damit, dass
...
\\
&=1-3\delta-2\delta -10\delta^2+O(\delta^3)
\end{align*}
Damit k"onnen wir jetzt $\lambda_+$ bis zur zweiten Ordnung berechnen:
\begin{align*}
\lambda_+
&=
\frac12\biggl((1-3\delta)+ (1-3\delta)-      2\delta-     10\delta^2\biggr)
+O(\delta^2)
\\
&=
1-3\delta-\delta-5\delta^2+O(\delta^3)
\end{align*}


Die beiden Terme in $\lambda_\pm$ 
