...
=
\underbrace{E(W(t)-W(s))}_{\displaystyle =0} \underbrace{E(W(s))}_{\displaystyle =0} + s,
\end{align*}
...
\rhead{Stochastische Differentialgleichungen}
...
\index{Integral!Paley-Wiener-Zygmund}
...
\biggr)
=
\int_0^T g'(t) \underbrace{E(W(t))}_{\displaystyle =0}\,dt=0.
\end{align*}
...
\\
&=
\int_0^T\int_0^Tg'(t)g'(s) \underbrace{E(W(t)W(s))}_{\displaystyle =t\wedge s}\,ds\,dt
\\
&=
...
&=
\int_0^T g'(t)\biggl(
\underbrace{[g(s)s]_0^t}_{\displaystyle =tg(t)}-\int_0^t g(s)\,ds+t(\underbrace{g(T)}_{\displaystyle =0}-g(t))
\biggr)\,dt
\\
&=
...
Zur Verf"ugung haben wir die drei Differenzen:
\begin{align*}
(b-a)^2
&=
b^2-2ab + a^2
...
%\index{Ito-Integral@It\^o-Integral}%
%\index{Integral!Ito@It\^o-}%
\end{definition}

Das It\^o-Integral von Stufenprozessen hat die folgenden Eigenschaften.
\begin{hilfssatz}
...
\\
&=
\sum_{k=0}^{m-1} E(G(t_k)) \underbrace{E(W(t_{k+1}) - W(t_k))}_{\displaystyle =0}=0.
\end{align*}
...
Das It\^o-Integral f"ur beliebige quadratintegrierbare Prozesse
...
\\
&=F_1F_2r^2+(G_1F_2+G_2F_1)\underbrace{\biggl(\int_0^rW\,dt + \int_0^rt\,dW\biggr)}_{\displaystyle = rW(r)}
\\
&\qquad
+G_1G_2\cdot \underbrace{2\int_0^r W\,dW}_{\displaystyle W(r)^2-r}
+ G_1G_2\underbrace{\int_0^r\,dt}_{\displaystyle =r}
\\
&= 
...
dX = g\,dW = \underbrace{0}_{\displaystyle F}\,dt
+
\underbrace{g}_{\displaystyle G}\,dW
\]
...
\\
&=
\underbrace{e^X}_{\displaystyle Y}\biggl(\frac12g^2\,dt + g\,dW\biggr)
\\
&=
\frac12g^2Y\,dt + gY\,dW.
\end{align*}
...
dX
=
\underbrace{-\frac12g^2}_{\displaystyle F}\,dt
+
\underbrace{g}_{\displaystyle G}\,dW.
\]
...
\\
&=
\underbrace{\biggl(-\frac12 g^2 + \frac12 g^2\biggr)}_{\displaystyle=0}\,dt
+
\underbrace{e^X}_{\displaystyle Y}g\,dW
\\
&=gY\,dW.
\end{align*}
...
Y(t)
=
\underbrace{e^{-\frac12 \int_0^tg^2\,d\tau}}_{\displaystyle \le 1}
\cdot e^{\int_0^tg\,dW}.
\]
...
\index{lineare stochastiche Differentialgleichung}
\index{Differentialgleichung!stochastische!lineare}
...
\[
dX=
\underbrace{\biggl( f-\frac12g^2\biggr)}_{\displaystyle F}\,dt
+
\underbrace{g}_{\displaystyle G}\,dW.
\]
...
\\
&=
e^X\biggl( f\underbrace{-\frac12g^2+\frac12 g^2}_{\displaystyle=0}\biggr)\,dt
+
e^Xg\,dW
\\
&=
...
=
-be^{-bt}
\underbrace{\biggl(V_0 + \sigma\int_0^te^{b\tau}\,dW\biggr)}_{\displaystyle =V}\,dt
+
e^{-bt}
\sigma e^{bt}dW
=
...
Die L"osung erlaubt, Erwartungswert und Varianz von $V(t)$ zu berechnen.
...
e^{-bt}E(V_0)
+
\sigma \underbrace{E\biggl(\int_0^t e^{-b(t-\tau)}\,dW\biggr)}_{\displaystyle=0}
=
e^{-bt}V_0.
\end{align*}
...
\index{Differentialgleichung!stochastische!quadratische}%
...
Die erste Gleichung von~\eqref{stochastisch:ornstein-uhlenbeck-dgl}
...
%Formel~\eqref{stochastisch:harmosz-explsg} ablesen:
\begin{equation}
\begin{pmatrix}
Y_1(t)\\Y_2(t)
\end{pmatrix}
...
\index{partielle Differentialgleichungen}%
...
\index{Funktion!charakteristische}%
\[
...
u(X(\tau_x))-u(X(0))
=
\int_0^{\tau_x} \frac12\underbrace{\Delta u}_{\displaystyle=0}\,ds=0.
\]
...
\index{harmonische Funktion!Charakterisierung}%
