...
Wir sehen $n(\vec{p})$ zwar als stetige aber allgemeinen nicht integrierbare Funktion von $R^3$ nach $R$.
...
%% TODO: Grafik Blick auf den Fisch unter Wasser, vermuteter Fisch, tatsächlicher Weg des Lichts.
...
$n_W \sin \alpha_W = n_L \sin \alpha_L$
\end{equation}

In diesem einfachen Szenario setzen wir die Brechzahl $n(\vec{p}) = 1$ f"ur jeden Punkt im Wasser und in der Luft - nur genau an der Wasseroberfl"ache gilt $n(\vec{p}) = \frac{n_Wasser}/{n_Luft}$.
...
Anstatt "ubereinanderliegender Luftschichten betrachten wir nun ineinanderliegende Ebene Sh"ahren mit konstantem Innen und Aussen-Radius.
