...
\index{Lichtbrechung in der Atmosphare@Lichtbrechung in der Atmosph\"are}%
\index{Refraktion, atmospharische@Refraktion, atmosph\"arische}%
...
Wir m"ochten basierend auf Jean Kovalevsky und P. Kenneth Seidelmanns 6. Kapitel in \cite[s. 121ff]{licht:astrometry} die scheinbare Verschiebung beobachteter Objekte durch atmosph"arische Refraktion und deren mathematischen Korrektur beschreiben. 
...
Wir vernachl"assigen s"amtliche Effekte, die weit entfernte Objekte auf den Weg dieses Lichtstrahls haben, und beschr"anken uns auf die Betrachtung der durch die Erdatmosp"are verursachten Ablenkung.
...
Diese  skalare Gr"osse kann f"ur jeden Punkt im Raum bzw. Ortsvektor angegeben werden und beschreibt das Verh"altnis der Brechungsindices zwischen zwei benachbarten Luftmassen.
...
Wir sehen $n(\vec{p})$ zwar als stetige aber im Allgemeinen nicht differenzierbare Funktion $ \mathbb{R}^3 \rightarrow \mathbb{R}$.
...
Wir wiederholen zun"achst das Brechungsgesetz nach Snellius und stellen die zwei einfachsten darauf basierenden Geometrien f"ur die anschliessend betrachteten Approximationen von $n(\vec{p})$ vor. 
...
Der Brechungsindex einer Luftschicht ist weiter nur von der H"ohe $y$ "uber Boden abh"angig und innerhalb einer Schicht konstant.
...
\Rightarrow \qquad\sin^2 (\alpha) = \frac{1}{y'(x)^2 + 1}
\end{equation}

Dies ist nicht ohne Division durch $y'(x)^2 + 1$ m"oglich, weshalb nun die Bedingung: $y'(x)^2 +1 \neq 0$ gilt, die aber immer erf"ullt ist.
Als n"achstes quadrieren wir das angepasste Brechungsgesetz \eqref{eq:13_1} und setzen \eqref{eq:13_2} ein:
\[
...
Wenn man nun $n'(y)=0$ in die Gleichung \eqref{eq:planar_DGL} einsetzt, erh"alt man: 
$y''(x) = 0.$
Auch aus unserer Gleichung ist also erkennbar, dass sich der Lichtstrahl bei konstantem $n$ nicht bricht.

Nun brauchen wir eine Funktion der Brechzahl in Abh"angigkeit von der H"ohe. 
...
\Rightarrow
\qquad
\frac{dn}{dy} = -\sigma \mu \cdot e^{-\sigma y}.
\]
Nun setzen wir dies in die Differentialgleichung ein, und erhalten:
\begin{equation} \label{eq:planar_DGL_n}
y''(x) = \frac{-\sigma}{\displaystyle\frac{1}{\mu e^{-\sigma y}} + 1} \cdot ( y'^2 + 1 ).
\end{equation}

Wie man anhand der Vorzeichen in \eqref{eq:planar_DGL} erkennen kann, kr"ummt sich der Lichstrahl immer in Richtung des gr"osseren $n$.
...
\begin{figure}
  ...
  \caption{Numerische L"osung der Differentialgleichung \eqref{eq:planar_DGL_n}. \label{fig:planares_modell1}}
\end{figure}
...
Entgegen der Meinung gewisser Minderheiten~\cite{licht:flatearthsociety} leben wir aber auf einem kugelf"ormigen Planeten, weshalb wir im n"achsten Abschnitt eine sph"arische Geometrie aufbauen m"ochten.

\section{Sph"arisches Modell}
\rhead{Sph"arisches Modell}
\index{spharisches Modell@sph\"arisches Modell}
...
Wir beginnen erneut mit dem Brechnungsgesetz von Snellius, welche wir auf unser sph"arisches Modell (Abbildung \ref{fig:sphere_skizze}) anpassen: 
...
\footnote{Verl"angert man $\overline{MN}$ nach unten und f"allt darauf ein Lot durch $O$ kann Gleichung \eqref{eq:snellius_spheric} direkt im entstehenden rechtwickligen Dreieck abgelesen werden, wenn man die L"ange der gemeinsamen Gegenkathete der beiden Sinusterme gleichsetzt.}:
...
\begin{align}
\tan^2 \alpha &= \frac{\sin^2\alpha}{\cos^2\alpha} = \frac{\sin^2\alpha}{1-\sin^2\alpha} = \frac{1}{\displaystyle\frac{1}{\sin^2\alpha}-1} = \biggl( \frac{r}{r'} \biggr)^2
\notag
\\
\Rightarrow \qquad\sin^2\alpha &= \frac{1}{\biggl( \displaystyle\frac{r'}{r} \biggr)^2 +1}.
\label{eq:sphere_sine}
\end{align}
\begin{figure} 
\centering
...
Nun quadrieren wir die Gleichung \eqref{eq:sphere_base} und setzen die Gleichung \eqref{eq:sphere_sine} ein:
...
Damit die Differentialgleichung auch korrekte Anfangsbedingungen besitzt, leiten wir die Gleichung nach $\varphi$ ab und erhalten:
\begin{align*}
0=
\frac{d}{d\varphi} \text{const}^2 
&=
\frac{d}{d\varphi}\frac{(n r)^2}{\biggl(\displaystyle\frac{r'}{r}\biggr)^2 + 1}
=
\frac{2 r^2 n r' n'}{\displaystyle\frac{r'^2}{r^2}+1}+\frac{2 r n^2 r'}{\displaystyle\frac{r'^2}{r^2}+1}-\frac{r^2 n^2 \biggl(\displaystyle\frac{2 r' r''}{r^2}-\frac{2 r'^3}{r^3}\biggr)}{\biggl(\displaystyle\frac{r'^2}{r^2}+1\biggr)^2}
\\
\Rightarrow\qquad
0
&=
\frac{2n r' r}{\displaystyle\frac{r'^2}{r^2}+1} \cdot \biggl( n' + n - \frac{n r r'' - n r'^2}{r'^2 + r^2} \biggr).
\end{align*}
Wie beim planaren Modell muss der Ausdruck in der Klammer $0$ sein, was wir nutzen k"onnen um eine Differentialgleichung f"ur $r''$ aufzustellen:
\begin{align}
&\Rightarrow&
n' + n - \frac{n r r'' - n r'^2}{r'^2 + r^2}
&=
0
\notag
\\
&\Rightarrow&
n' r'^2  + n' r^2 + n r'^2 + nr^2 
&=
n r r'' - n r'^2
\notag
\\
&\Rightarrow&
n' r'^2 + n' r^2 + 2 n r'^2 + n r^2 
&=
n r r''
\notag
\\
...
\label{eq:sphere_allg}
\end{align}
Damit haben wir eine allgemeine Differentialgleichung f"ur Lichstrahlen
in einer sph"arisch symmetrischen Atmosph"are erhalten.
...
n'(r) = -\sigma \mu e^{-\sigma (r - r_0)}.
\]
Nun setzen wir dies in die Differentialgleichung \eqref{eq:sphere_allg} ein und berechnen die L"osung numerisch (Abbildungen \ref{fig:sphaerisches_modell2} und \ref{fig:sphaerisches_modell3}). 
...
Um diese Bedingungen zu finden, starten wir mit der Gleichung \eqref{eq:sphere_allg}:
\[
...
\index{Flat Earth Society}%
\index{Weltbild}

Wir m"ochten ein solches $n$ finden und untersuchen als erstes eine exponentielle Approximation der Atmosph"are mit:
\[
n(r) = 1 + \mu \cdot e^{-\sigma (r-r_0)}
\qquad\Rightarrow\qquad
n'(r) = -\mu \sigma \cdot e^{-\sigma (r-r_0)}.
\]
Wir setzen diese Funktionen nun in die obige Bedingung ein und erhalten:
\[
\mu \sigma \cdot e^{-\sigma (r-r_0)} = 1 + \mu \cdot e^{-\sigma (r-r_0)}
\qquad \Rightarrow \qquad
\mu \cdot e^{-\sigma (r-r_0)} \cdot (\sigma - 1) = 1
\qquad \Rightarrow \qquad
...
n(r) =
\begin{cases}
1 + \mu \cdot (r_0 - r)^2 &\quad\text{wenn } r < r_0 \\
1                         &\quad\text{sonst}
\end{cases}
\qquad
\text{und}
...
=
\begin{cases}
2\mu \cdot (r - r_0) &\quad\text{wenn } r < r_0 \\
0                    &\quad\text{sonst.}
\end{cases}
\]
...
Wenn aber wie vorhin erneut der Fehler simuliert wird, ist die Abweichung viel kleiner als bei der exponentiellen Approximation. 
...
\begin{figure}
  ...
  \caption{Numerische L"osung der exponentiellen Approximation mit $r(0) = r_0 = 1$, $r'(0) = 0$, $\sigma = 2$, $\mu = 1$. \label{fig:sphere_special1}}
...
\begin{itemize}
  ...
  \item Luftfeuchtigkeit: Der Wasserdampfanteil in der Luft beeinflusst die Brechzahl relativ stark. Jedoch variiert diese sehr stark mit dem Wetter. Deshalb wird dieser Einfluss vernachl"assigt. 
  \item Luftdruck: Der Luftdruck l"asst sich approximativ relativ gut modellieren. Aus der Physik ist bekannt, dass bei konstanter Temperatur der Druck bei Gasen mit der H"ohe exponentiell abnimmt. 
  \item Temperatur: Wie in Abbildung \ref{fig:athmosphere_profile} erkennbar ist, ver"andert sich die Temperatur in Abh"angigkeit von der H"ohe sehr stark. Es ist jedoch sehr schwer, diesen Verlauf zu modellieren. Deshalb wird die Temperatur (vorerst) im Modell vernachl"assigt.
\end{itemize}

\begin{figure}
  \centering
  %\includegraphics[scale=0.24]{licht/images/athmosphereProfile.png}
  \includegraphics[scale=0.33]{licht/images/athmosphereProfile.png}
  
  \caption{Druck, Dichte, Temperatur und Schallgeschwindigkeit in Abh"angigkeit von der H"ohe (aus \cite{licht:wikipedia_atmospheric_temperature}). \label{fig:athmosphere_profile}}
\end{figure}

Dank diesen Vereinfachungen l"asst sich die Brechzahl in Abh"angigkeit von $r$ folgendermassen beschreiben:
$$n(r) = 1 + \mu \cdot e^{-\sigma (r - r_0)}$$
...
\begin{figure}
  ...
  \caption{In der Praxis genutzte Approximationskurve f"ur Atmosph"arische
Refraktion verglichen mit den Simulationsergebnissen. \label{fig:real_comparison}}
\end{figure}

\section{Fazit und Zusammenfassung}
Basierend auf dem Brechungsgesetz von Snellius haben wir uns zwei verschiedene Geometrien angesehen und verschiedene Modellierungen der Brechzahl darin simuliert und numerisch ausgewertet. 
Die Ergebnisse liegen erfreulich nahe an den in der Praxis zur Bereinigung benutzten Werte.
Die mathematischen Eigenschaften der verwendeten Gleichungen zeigen erste Grenzf"alle an, welche tats"achlich auch besonderen Situationen in der Natur entsprechen.
