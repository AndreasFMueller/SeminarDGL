...
\index{Bacon, Sir Francis von Verulam}%
\end{flushright}
...
\index{de Sitter, Willem}%
\index{Friedmann, Alexander}%
\index{Lema\^itre, Georges}%
...
\end{equation}
Der $\kappa$-Parameter beschreibt die Krümmung des Raumes und ist in Tabelle \ref{friedmann:GeometrieDesRaumes} erklärt. Die Krüm\-mung des Raumes kann durch Messen von Winkelsummen im Dreieck bestimmt werden.
\begin{table}
\centering
\begin{tabular}{|>{$}r<{$}|>{$}l<{$}|>{$}c<{$}|}
\hline
...
\end{table}
Mathematisch gesehen hat der $\kappa$-Term abgesehen vom Vorzeichen die gleiche Bedeutung wie die Gesamtenergie des Universums, die in \eqref{friedmann:EnergieerhaltungUniversum} behandelt wurde, nämlich:
\begin{itemize}
